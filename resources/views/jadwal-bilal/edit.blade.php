    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form action="{{ route('jadwal-bilal.update', $jadwal) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 border rounded-md p-3 max-h-64 overflow-y-auto bg-white" id="container-bilal">
                            @foreach ($pengurus as $p)
                                <label class="item-bilal flex items-center justify-between p-2 rounded hover:bg-blue-50 cursor-pointer text-sm border border-gray-100 transition"
                                    ...
                                    <input type="file" name="foto[{{ $p->id }}]" accept="image/*" class="foto-petugas {{ in_array($p->id, $anggotaTerpilih) ? '' : 'hidden' }} w-32 text-[10px]" title="Ganti foto petugas">
                                    @if ($p->asal === 'guru')
